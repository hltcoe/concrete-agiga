...
public class ConstituentTest {
  ...
  public void testEntityType() throws ConcreteException {
      ...
      assertTrue("entity type is " + e.getType() +", not PERSON",
                 e.getType().equals("PERSON"));
  }
  ...
  public void testMentionTypes() throws ConcreteException {
      ...
      assertTrue("expected 5 entities, got " + ems.getMentionList().size(), 
                 ems.getMentionList().size() == 5);
      int numOther = 0;
      int numPerson = 0;
      for(EntityMention em : ems.getMentionList()) {
          String et = em.getEntityType();
          if(et.equals("Unknown") ) numOther++;
          else if(et.equals("PERSON")) numPerson++;
          else {
              assertTrue("mention type is " + em.getEntityType() +", not PERSON or Unknown",
                         false);
          }
      }
      assertTrue("saw " + numOther + " Unknown types, expected 2",
                 numOther == 2);
      assertTrue("saw " + numPerson + " PERSON types, expected 3",
                 numPerson == 3);
  }
